...
def _is_remote_node_url(node_url: str) -> bool:
    """Return True only for non-empty URLs that don't point at localhost."""
    return bool(node_url) and "localhost" not in node_url and "127.0.0.1" not in node_url


def _classify_png_entry(val: dict, p: str, path_inline: dict, path_artifact: dict) -> None:
    """Slot a pngBase64 value into the correct map: inline bytes or artifact-path reference."""
...
def _process_remote_path_entry(
    ...
    if not _is_remote_node_url(node_url):
        return
    ...
    _classify_png_entry(val, p, path_inline, path_artifact)
...
def _try_local_file_preview(att: dict, path: str) -> bool:
    """Set preview URL from a local filesystem path. Returns True if the file exists."""
    import os as _os  # noqa: PLC0415
    from urllib.parse import quote as _quote  # noqa: PLC0415
...
def _try_artifact_path_preview(att: dict, path: str, path_artifact: dict) -> bool:
    """Resolve via an alternate artifact copy on the local filesystem. Returns True on success."""
    import os as _os  # noqa: PLC0415
    from urllib.parse import quote as _quote  # noqa: PLC0415
...
def _try_inline_or_proxy_preview(
    att: dict, path: str, path_inline: dict, path_to_node: dict, shot_dir: str
) -> None:
    """Try inline base64 save, then fall back to the remote-proxy URL."""
    from urllib.parse import quote as _quote  # noqa: PLC0415
...
def _resolve_attachment_preview(att, path_to_node: dict, path_inline: dict,
    ...
    if not path:
        return
    if _try_local_file_preview(att, path):
        return
    if _try_artifact_path_preview(att, path, path_artifact):
        return
    _try_inline_or_proxy_preview(att, path, path_inline, path_to_node, shot_dir)
...
def _try_register_one_artifact(sid: str, sr: dict, uri_by_id: dict, db: str | None, host_db) -> bool:
    """Attempt to register a single step result as a catalog artifact. Returns True if registered."""
...
def _register_step_artifacts(result: dict, db: str | None, host_db) -> int:
    ...
    return sum(_try_register_one_artifact(sid, sr, uri_by_id, db, host_db) for sid, sr in results.items())
