...
moves ``{uri, payload}`` to where `v2.run` executes it. This module drives the
*same* registry over five representative layers so every implementation shape is
visible and testable:

- inprocess  - call the library directly (`v2.run`)
...
- http       - `v2_service` over a tiny HTTP `/run` worker
- grpc       - `v2_grpc` over a generic gRPC `Run`
...
def build_registry() -> dict:
    return v2.compile_registry(json.loads((ROOT / "registry.bindings.json").read_text(encoding="utf-8")))
...
def run_inprocess(uri: str, payload: dict, registry: dict, mode: str = "execute") -> dict:
    return v2.run(uri, registry, payload=payload, mode=mode, policy=ALLOW if mode == "execute" else None)
...
def run_queue(uri: str, payload: dict, registry: dict, timeout: float = 10.0) -> dict:
    ...
    def consumer():
        ...
        replies.put(v2.run(message["uri"], registry, payload=message["payload"], mode="execute", policy=ALLOW))
...
def serverless_handler(event: dict, registry: dict) -> dict:
    return v2.run(event["uri"], registry, payload=event.get("payload"), mode="execute", policy=ALLOW)
    ...
    def make_handler():
        class Handler(BaseHTTPRequestHandler):
            ...
            def do_POST(self):
                ...
                self._send(200, v2.run(body["uri"], registry, payload=body.get("payload"), mode="execute", policy=ALLOW))
...
def run_via(transport: str, uri: str, payload: dict, registry: dict) -> dict:
    ...
    if transport == "http":
        ...
        try:
            return v2_service.call(uri, payload, registry)
        finally:
            server.shutdown()
            os.environ.pop("URI_SERVICE_MAP", None)
    if transport == "grpc":
        from urirun import v2_grpc  # optional dependency (grpcio)
        server, port = v2_grpc.serve(registry, host="127.0.0.1", port=0, policy=ALLOW, mode="execute", block=False)
        os.environ["URI_GRPC_MAP"] = json.dumps({target: f"127.0.0.1:{port}"})
        try:
            return v2_grpc.call(uri, payload, registry)
        finally:
            server.stop(0)
