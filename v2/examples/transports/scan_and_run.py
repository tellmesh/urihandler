...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("source", help="project dir, v2 bindings file, or registry document")
    ...
    registry = v2.load_registry_arg(args.source)          # <-- the scan/compile step
    payload = json.loads(args.payload)
    mode = "execute" if args.execute else "dry-run"

    if args.transport == "inprocess":
        policy = {"execute": {"allow": [args.uri]}} if args.execute else None
        result = v2.run(args.uri, registry, payload=payload, mode=mode, policy=policy)
    elif args.transport == "http":
        result = v2_service.call(args.uri, payload, registry, target=args.target, mode=mode)
    else:
        from urirun import v2_grpc
        result = v2_grpc.call(args.uri, payload, registry, target=args.target, mode=mode)
