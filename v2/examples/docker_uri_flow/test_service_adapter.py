...
def registry() -> dict:
    ...
    return v2.compile_registry({"bindings": bindings})
...
def test_dry_run_plans_the_http_call_without_network():
    env = v2_service.call("python://python-worker/text/normalize", {"text": "Hi"}, registry(), mode="dry-run")
...
def test_schema_validation_runs_before_dispatch():
    env = v2_service.call("python://python-worker/text/normalize", {}, registry())  # missing required text
    assert env["ok"] is False
    assert env["error"]["type"] == "schema"


def test_unknown_uri_is_a_registry_error():
    env = v2_service.call("python://python-worker/text/missing", {"text": "x"}, registry())
    assert env["ok"] is False
...
def test_service_dispatch_calls_live_workers():
    ...
    try:
        for port in ports.values():
            wait_health(port)
        reg = registry()
        normalized = v2_service.call("python://python-worker/text/normalize", {"text": " Supplier Report "}, reg)
        assert normalized["ok"] is True
        assert normalized["result"]["normalized"] == "supplier report"

        slug = v2_service.call("node://node-worker/text/slugify", {"text": "Supplier Report June 2026"}, reg)
