...
Both transports execute through `v2.run` (so bindings are authoritative) and
...
def serve_grpc() -> None:
    ...
    v2_grpc.serve(REGISTRY, host="0.0.0.0", port=PORT, policy=ALLOW, mode="execute", block=True)
