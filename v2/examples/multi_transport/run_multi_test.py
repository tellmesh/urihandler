...
from urirun import _registry as reglib, v2, v2_grpc, v2_service
...
def main() -> int:
    ...
    rpc_bindings = v2_grpc.list_routes(RPC)["bindings"]
    ...
    registry = v2.compile_registry({"bindings": {**web_bindings, **rpc_bindings}})
    ...
    normalized = v2_service.call("web://reports/text/normalize", {"text": "Supplier Report June 2026"}, registry)
    assert normalized["ok"], normalized
    slug = v2_service.call("web://reports/text/slugify", {"text": normalized["result"]["stdout"].strip()}, registry)
    assert slug["ok"], slug
    rendered = v2_grpc.call("rpc://reports/report/render", {"slug": slug["result"]["stdout"].strip()}, registry)
