...
class ArtifactAdoptionTests(unittest.TestCase):
    def test_artifact_scan_builds_v2_bindings_from_common_standards(self):
    ...
    def test_cli_scan_validate_compile_and_run(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            subprocess.run(
                [sys.executable, "-m", "urirun.v2", "validate", str(bindings_path)],
                check=True,
                capture_output=True,
                text=True,
            )
    ...
    def test_cli_add_pypi_and_command_binding_in_one_line(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            subprocess.run(
                [sys.executable, "-m", "urirun.v2", "compile", str(bindings_path), "--out", str(registry_path)],
                check=True,
            )
...
class HtmlAppTests(unittest.TestCase):
    def test_html_backend_dispatches_v2_runtime(self):
