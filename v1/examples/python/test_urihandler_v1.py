...
from urirun.v1 import compile_registry, load_registry_arg, run

BINDINGS = Path(__file__).resolve().parents[1] / "json" / "bindings.v1.example.json"
...
class CliTests(unittest.TestCase):
    def test_cli_compile_and_run_dry(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            subprocess.run(
                [sys.executable, "-m", "urirun.v1", "compile", str(BINDINGS),
                 "--out", str(registry_path), "--generated-at", "2026-06-19T00:00:00.000Z"],
                check=True,
            )
            result = subprocess.run(
                [sys.executable, "-m", "urirun.v1", "run", "media://local/video/transcode",
