# Extending one registry with new endpoints (v1)
...
urirun run 'ops://local/notify/send' /tmp/extend.registry.json \
  --payload '{"channel":"deploys","message":"v1 shipped"}' \
  --allow 'ops://local/notify/send' --execute
#   stdout: notify -> #deploys: v1 shipped
```
...
Covered by `v1/examples/python/test_extend.py` (run as part of `make test-v1`):

```bash
PYTHONPATH=adapters/python python3 -m unittest discover -s v1/examples/python -p 'test_*.py'
```
