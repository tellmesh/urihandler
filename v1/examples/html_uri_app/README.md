...
This console demonstrates the v1 runtime. The key idea is **named parameter
...
> the exact command — the same command the `urirun.v1` CLI runs for real.
> The GitHub GET endpoint performs a real request.

## Run

```bash
bash v1/examples/html_uri_app/run.sh
...
- `uri-runtime-v1.js` - browser-safe v1 runtime (param binding + policy + preview), mirrors `urirun/v1.py`
