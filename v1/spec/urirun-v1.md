...
`urirun v1` makes real tools easy to drive: ffmpeg, kubectl, git, docker.

Real command-line tools need *named* flags and values, so v1 provides parameter
...
```bash
urirun-v1 compile bindings.v1.json --out .urirun/registry.merged.json
urirun-v1 list .urirun/registry.merged.json --allow 'media://**'
urirun-v1 run media://local/video/transcode bindings.v1.json --payload '{"input":"a.mp4","output":"b.mp4"}'
urirun-v1 run media://local/video/transcode bindings.v1.json --payload '{"input":"a.mp4","output":"b.mp4"}' --allow 'media://**' --execute
```
